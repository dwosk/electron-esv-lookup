...
const TitleBar = () => {
  ...
  let windowIcon;
  if (useDefault) {
    windowIcon = <button class="ui-btn maximize" onClick={() => { handleClick('maximize'); }}>
        			     <svg viewBox="0 0 10 10"><path d="M0,0v10h10V0H0z M9,9H1V1h8V9z" /></svg>
        		     </button>
  } else {
    windowIcon = <button class="ui-btn restore" onClick={() => { handleClick('restore'); }}>
  ...
  return (
    <div class="ui-titlebar">
    	<div class="ui-titlecontrols">
    		<button class="ui-btn minimize" onClick={() => { handleClick('minimize'); }}>
    			<svg x="0px" y="0px" viewBox="0 0 10.2 1"><rect x="0" y="50%" width="10.2" height="1" /></svg>
    		</button>
        {windowIcon}
        <button class="ui-btn close" onClick={() => { handleClick('quit'); }}>
